...
def etablirplateau():
    ...
    for row in range(len(GRILLE_ALLIE)):
        for col in range(len(GRILLE_ALLIE[row])):
            c = CAN_ALLIE.create_rectangle(col * CELL_SIZE,row * CELL_SIZE,col * CELL_SIZE + CELL_SIZE,row * CELL_SIZE + CELL_SIZE,outline="black") #utilisation de la taille des cellules pour definir le point d'origine en haut a gauche, vers le point d'arrivée en bas a droite
    ...
    for row in range(len(GRILLE_ENNEMI)):
        for col in range(len(GRILLE_ENNEMI[row])):
            c = CAN_ENNEMI.create_rectangle(col * CELL_SIZE,row * CELL_SIZE,col * CELL_SIZE + CELL_SIZE,row * CELL_SIZE + CELL_SIZE,outline="black")
